...
def rule_check(line, ind, number, index_check, entities, entao):
...
def unidades(line, ind, other = False):
    ...
    while index_check < len(line):
        ...
        if index_check == ind:
            ...
            pass
        if entao == None:
            entao = rule_check(line, ind, number, index_check, entities, entao)
        if entao:
            if sum == -1:
                sum = 1
            else:
                final = rule_check(line, ind, number, index_check, entities, entao)
                        ...
                        for ind in range(len(line.split())):
                            if re.findall("\d+", line.split()[ind]):
                                ...
                                try:
                                    ...
                                    words = time_finder(line.split(), ind)
                                    if not words:
...
text = """"500mg of Fulvestrant will be given IM on days 1, 15, 28, then every 4 weeks as per standard of care (SOC) and 160mg of Enzalutamide will be given, in conjunction with Fulvestrant, PO daily.,""""
...
"""
NCT02953860.json ['Aspartate aminotransferase', '<', '3', 'times', 'ULN']           Aspartate aminotransferase (AST) or alanine aminotransferase (ALT) < 3 times ULN or < 5 times ULN if patient has documented liver metastases,

NCT02953860.json ['ULN', '<', '5', 'times', 'ULN']           Aspartate aminotransferase (AST) or alanine aminotransferase (ALT) < 3 times ULN or < 5 times ULN if patient has documented liver metastases,

"""

"""Cagar nisto
NCT00003782.json {('(cT1-3', 'N0-1'), ('(cT1-3', 'IIIA')}           Stage I, II, or IIIA (cT1-3, N0-1, M0),

NCT00003782.json [('0-1', 'N')]           Stage I, II, or IIIA (cT1-3, N0-1, M0),

NCT00003782.json None           Stage I, II, or IIIA (cT1-3, N0-1, M0),
"""


"(line, n_position, number, unidade, of, comparison, bigger/less, < > side, times, times what,)"
